...
### 9.1 Azure OpenAI 통합 추가
- Azure OpenAI API를 사용하여 고품질 LLM 기능을 제공합니다.
- API 키 및 엔드포인트 설정을 `.env` 파일을 통해 안전하게 관리할 수 있게 했습니다.
- 서비스별로 다양한 모델(GPT-4, GPT-3.5 등)을 선택할 수 있는 기능을 추가했습니다.

### 9.2 LangGraph 워크플로우 도입
- 수퍼바이저 서비스에 LangGraph 기반 에이전트 워크플로우를 구현했습니다.
- 자동차 진단 에이전트와 정비 조언 에이전트를 별도의 노드로 분리하여 전문성을 높였습니다.
- 사용자 메시지 의도에 따라 적절한 에이전트로 라우팅하는 기능을 추가했습니다.
- 필요한 지식을 자동으로 검색하여 응답의 정확성을 높였습니다.

### 9.3 LLM Registry 서비스 강화
- 다양한 LLM 서비스를 등록하고 관리할 수 있는 레지스트리 서비스를 개선했습니다.
- 텍스트 완성, 채팅 완성, 함수 호출 등 다양한 API를 통합 관리합니다.
- 서비스별 기본 파라미터 및 기능 정보를 제공하여 유연한 LLM 활용이 가능합니다.

### 9.4 프론트엔드 LLM 서비스 선택 기능
- 사용자가 대화에 사용할 LLM 서비스를 선택할 수 있는 UI를 추가했습니다.
- 각 LLM 서비스의 기능과 모델 정보를 확인할 수 있습니다.
- API 키 상태를 확인하여 서비스 가용성을 쉽게 파악할 수 있게 했습니다.
...
8. **LLM 서비스 확장**: 다양한 LLM 제공자(OpenAI, Anthropic, Cohere 등)를 추가하여 서비스 다양성을 높일 수 있습니다.

9. **벡터 데이터베이스 통합**: 자동차 정비 관련 문서를 벡터화하여 저장하고, 검색 기능을 강화할 수 있습니다.

10. **멀티모달 기능 추가**: 이미지 인식 기능을 강화하여 사용자가 업로드한 차량 사진을 분석할 수 있습니다. 